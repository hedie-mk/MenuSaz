...
export default function Table({isLoading , filteredItem , tHead} : TableProps){
            ...
            {isLoading ? (
              ...
              filteredItem?.map((item : any) => (
                <tr key={item.id} className="border-b">
                  ...
                  <td className="px-4 py-2 text-[#444]">{item.category}</td>
                  ...
                  <td className="px-4 py-2 flex gap-2 items-center">
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input type="checkbox" className="sr-only peer" checked={item.status === 'active'} readOnly />
